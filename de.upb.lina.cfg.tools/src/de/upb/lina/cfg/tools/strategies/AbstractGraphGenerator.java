...
public abstract class AbstractGraphGenerator implements IGraphGenerator {
   ...
   public abstract EMemoryModel getMemoryModel();
   ...
   public ControlFlowDiagram createGraph() {
      ...
      this.graph.setMemoryModel(this.getMemoryModel().getModelId());
   ...
   /**
    * Overwrite this method to add special treatment of transitions, e.g., for read, write,
    * atomicRmw instructions. This implementation handles all cases of instructions that DO NOT NEED
    * store buffer modification. Hence, all following locations will have a StoreBuffer that is
    * identical to currentLocation.
    * 
   ...
   protected void addInstructionTransitions(ControlFlowLocation currentLocation, Instruction nextInstruction) {
      if (nextInstruction instanceof Branch) {
         ...
         if (branch.getElseDestination() != null) {
            ...
            // already exist
            ControlFlowLocation trueCaseNextLocation = getLocationByPcAndBuffer(trueCasePC, currentLocation.getBuffer());
            ControlFlowLocation elseCaseNextLocation = getLocationByPcAndBuffer(elseCasePC, currentLocation.getBuffer());

            if (trueCaseNextLocation == null) {
               trueCaseNextLocation = createControlFlowLocation(trueCasePC, cloneStoreBuffer(currentLocation.getBuffer()));
               this.toBeProcessedLocations.add(trueCaseNextLocation);
            }

            if (elseCaseNextLocation == null) {
               elseCaseNextLocation = createControlFlowLocation(elseCasePC, cloneStoreBuffer(currentLocation.getBuffer()));
            ...
            Instruction targetInstruction = GraphUtility.getInstructionByLabel(this.function, targetInstructionLabel);
         ...
         ControlFlowLocation defaultCaseLocation = getLocationByPcAndBuffer(defaultCasePC, currentLocation.getBuffer());
         if (defaultCaseLocation == null) {
            defaultCaseLocation = createControlFlowLocation(defaultCasePC, cloneStoreBuffer(currentLocation.getBuffer()));
            this.toBeProcessedLocations.add(defaultCaseLocation);
         }

         GuardedTransition defaultCase = createGuardedTransition(currentLocation, switchInstruction, defaultCaseLocation);
         ...
         for (SwitchCase sc : switchInstruction.getCases()) {
            ...
            if (caseTargetLocation == null) {
               caseTargetLocation = createControlFlowLocation(casePC, cloneStoreBuffer(currentLocation.getBuffer()));
         ...
         ControlFlowLocation nextLocation = getLocationByPcAndBuffer(currentLocation.getPc() + 1, currentLocation.getBuffer());
         if (nextLocation == null) {
            nextLocation = createControlFlowLocation(currentLocation.getPc() + 1, cloneStoreBuffer(currentLocation.getBuffer()));
            this.toBeProcessedLocations.add(nextLocation);
         }
