...
public class Dependent_Write_Read_1_1_rev extends TSO_Test {
	...
	public void setUp() throws Exception {
		testLocation = "ReverseLS/Test_Dependent_Write_Read_1_1_rev.s";
		super.setUp();
	}
	...
	public final void testCreateReachibilityGraph() {
		...
		for (ControlFlowLocation l : locs) {
			...
			for (Transition t : l.getOutgoing()) {
				if (!t.eClass().equals(
						ControlflowPackage.eINSTANCE.getFlushTransition())) {
					if (t.getInstruction().eClass()
							.equals(LlvmPackage.eINSTANCE.getFence())) {
						fenceTransition = t;
					}
				}
			}
		...
		for (ControlFlowLocation l : nonEmptyBuffers) {
			...
			boolean isValidBuffer = buffer.equals("L" + l.getPc()
					+ " <(r1: b)>");
		...
		if (fenceTransition != null) {
			for (ControlFlowLocation l : diag.getLocations()) {
				if (l.getPc() > fenceTransition.getSource().getPc()
						|| l.getIncoming().contains(fenceTransition)) {
