...
public class AllInOneWizard extends ConfigurationWizardUsingChecks implements INewWizard {
   ...
   /**
    * Creates a {@link TransformationOpertion} defined by the parameters specified by the user.
    * 
   ...
   private TransformationOperation createTransformationOperationFromUserInput() {
      ...
      TransformationOperation transformationOperation = new TransformationOperation(outputFolderPath, outputFileName, outputFileExtension,
   ...
   /**
    * Creates a {@link TransformationConfiguration} according to the parameters selected by the user
    * for the given AST.
    * 
    * @param ast to create the configuration for
    * @return transformation confgiguration
    */
   private TransformationConfiguration createTransformationConfigurationFromUserInput(LLVM ast) {
   ...
   /**
    * Creates a list of store buffer graphs fitting the given AST. As semantics the user selected
    * semantics are used.
    * 
   ...
   private List<ControlFlowDiagram> createStoreBufferGraphsFromAstAndUserInput(LLVM ast) {
      int memoryModelId = allInOneConfigurationPage.getSelectedSemantics();
      EMemoryModel memoryModel = EMemoryModel.getMemoryModelById(memoryModelId);
      List<ControlFlowDiagram> storeBufferGraphs = TransformationUtil.createCFGFromAst(memoryModel, ast);
      return storeBufferGraphs;
   }


   /**
    * Loads the LLVM file specified by the user and creates the according AST for it.
    * 
   ...
   public void restartChecks() {
      ...
      if (ast == null) {
         ...
         EMemoryModel memoryModel = EMemoryModel.getMemoryModelById(allInOneConfigurationPage.getSelectedSemantics());
